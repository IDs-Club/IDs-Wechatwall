...
class WeiXin
{
	...
	public function login() {
		$url = "https://admin.wechat.com/cgi-bin/login?lang=en_US";
	...
	{
		$url = "https://admin.wechat.com/cgi-bin/getcontactinfo?t=ajax-getcontactinfo&lang=en_US&token={$this->webToken}&ajax=1&fakeid=$fakeId";
		$re = $this->lea->submit($url, array(), $this->cookie);
		$result = json_decode($re['body'], 1);
		if(!$result) {
			$this->login();
	...
	public function getLatestMsgs() {
		...
		$url = "https://admin.wechat.com/cgi-bin/getmessage?t=wxm-message&token={$this->webToken}&lang=en_US&count=1";
	...
	public function getPicture($fakeId){
		$url = "https://admin.wechat.com/cgi-bin/getheadimg?token={$this->webToken}&fakeid={$fakeId}";
		$re = $this->lea->get($url, $this->cookie);
